...
het_varsIn <-  args[7]
transform <-  args[8]
transform.rankNorm <-  args[9]
transform.rescale <-  args[10]
kinship.matrix <- unlist(strsplit(sub('kinship=','',args[11]),' '))
...
## Setup
#source("/chru/analysis/dnanexus/github/genesis_nullmodel4/genesis_nullmodel/resources/home/dnanexus/pipelineFunctions.R")
source("/home/dnanexus/pipelineFunctions.R")
sessionInfo()
covariates <- split.by.comma(covariate.string)  

## phenotype 
phenotype.data <- read.csv('phenofile', header=TRUE, as.is=TRUE)
...
save(nullmod,pheno,file='results')
