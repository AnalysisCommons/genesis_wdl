...
genome_build = args[11]
pass_only = as.logical(args[12])
imputed = as.logical(args[13])
neig = as.numeric(args[14])
ntrace = as.numeric(args[15])
interaction = args[16]

if (interaction == 'FALSE') interaction = NULL
# GLOBAL VARIABLES
collapsing.tests <- c("Burden", "SKAT", "fastSKAT", "SMMAT", "fastSMMAT", "SKATO")
test.type.vals <- c("Single",collapsing.tests)
test.stat.vals <- c("Score",  "Score.SPA")
genome_build.vals <- c("hg19",  "hg38A")
					...
					## Collapse test
				 	...
				 	if (SW_T || AGG_T) {
							...
							res <- assocTestAggregate( iterator, 
																	in_nullmod, 
																	weight.beta = weights,
																	test=test.type,   genome.build = genome_build, imputed=imputed,
						 ...
						 generes <- assocTestSingle(iterator,  test =  test.stat, in_nullmod, genome.build = genome_build,imputed=imputed,verbose=T)
						 #via <- variantInfo(svd, alleles = TRUE, expanded=FALSE)
