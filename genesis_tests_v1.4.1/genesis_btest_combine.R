...
test.type  <-  args[4] # Burden, Single, SKAT etc
...
interaction = args[16]
return_variants = as.logical(args[17])
...
if(!test.type %in% collapsing.tests){
	...
	AGG_T = T
	if(!USE_AGG){
		warning("Aggregate testing requires an aggregation file, unless running a sliding window")
	}
}
...
doOne = function(idx,in_nullmod) {
	print(paste0(idx,' iter of ',length(dat)))
	##############
